...
namespace BlogProject.Implementation.Validators.Users
{
    ...
    {
        public RegisterUserValidator(BlogProjectContext _context)
        {
            RuleFor(x =>x.Username).NotEmpty().MinimumLength(2).Matches("[A-z0-9]*").WithMessage("Username must contains numbers and letters")
                .Must(x => !_context.Users.Any(user => user.Username == x)).WithMessage("Username is already taken");

            RuleFor(x => x.Password).NotEmpty().MinimumLength(6).Matches("[A-z0-9]*").WithMessage("Password must contains numbers and letters");
